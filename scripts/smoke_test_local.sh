...
# Allow forcing regeneration with REGEN_CERTS=1
if [ "${REGEN_CERTS:-0}" = "1" ] || ! [ -f "$CERT_DIR/ca.crt" -a -f "$CERT_DIR/server.crt" -a -f "$CERT_DIR/client.crt" ]; then
  ...
  # Server cert (ecall pool) — create SAN config and sign with CA
...
EOF
  ...
  # Create CSR with CN matching the host (127.0.0.1)
  openssl req -new -key "$CERT_DIR/server.key" -out "$CERT_DIR/server.csr" \
    -subj "/C=VN/ST=HCM/L=ThuDuc/O=UIT/OU=NT219/CN=127.0.0.1"

  # Sign server cert with SAN extfile
...
# Start mongod if available, else require MONGO_URI env
if command -v mongod >/dev/null 2>&1; then
  ...
  for i in {1..30}; do
    if command -v mongosh >/dev/null 2>&1; then
      mongosh --quiet --eval 'db.adminCommand({ ping: 1 })' ${MONGO_URI}/admin >/dev/null 2>&1 && break
    else
      # fallback to mongo client if mongosh not present
      mongo --eval 'db.adminCommand({ ping: 1 })' ${MONGO_URI}/admin >/dev/null 2>&1 && break
    fi
    sleep 1
  done
  echo "[smoke-local] mongod started (pid=$MONGOD_PID)"
else
  if [ -z "${MONGO_URI:-}" ]; then
    echo "[smoke-local] mongod not found and MONGO_URI not set. Please install mongod or set MONGO_URI to a running MongoDB."
    exit 1
  fi
  echo "[smoke-local] using provided MONGO_URI=${MONGO_URI}"
  echo "[smoke-local] mongod not found — running ECALL pool in mock-data mode (no seeding)"
  SKIP_SEED=1
  T8_POOL_DATA_MODE="auto"
  MONGO_URI=""
fi
...
if [ "$SKIP_SEED" -eq 0 ]; then
  ...
  echo "[smoke-local] skipping seeding (mongod not available). ECALL pool will use mock dataset."
fi

echo "[smoke-local] starting ecall task pool (with TLS)"
T8_POOL_HOST=127.0.0.1 T8_POOL_PORT=9091 T8_POOL_DATA_MODE="$T8_POOL_DATA_MODE" MONGO_URI=${MONGO_URI} T8_SSL_CERT="$T8_SSL_CERT" T8_SSL_KEY="$T8_SSL_KEY" T8_SSL_CA="$T8_SSL_CA" "$VENV_PY" enclave/enclave/ecall_pool.py &
ECALL_PID=$!

echo "[smoke-local] starting router (uvicorn)"
ECALL_POOL_URL="$ECALL_POOL_URL" ROUTER_CLIENT_CERT="$ROUTER_CLIENT_CERT" ROUTER_CLIENT_KEY="$ROUTER_CLIENT_KEY" T8_SSL_CA="$T8_SSL_CA" "$VENV_PY" -m uvicorn router.main:app --host 127.0.0.1 --port 8000 &
ROUTER_PID=$!
...
RESP=$(curl -sS -w "\n%{http_code}" -X POST http://127.0.0.1:8000/query -H "Authorization: Bearer ${TOKEN}" -H "Content-Type: application/json" -d '{"query_type":"sum_vien_phi","filters":{}}' || true)
